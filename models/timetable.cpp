...
int Timetable::classroomId() const
{
    return d->classroomId;
}
...
int Timetable::timeId() const
{
    return d->timeId;
}

void Timetable::setTimeId(int timeId)
{
    d->timeId = timeId;
}
...
QJsonArray Timetable::getAllJson()
{
    ...
    if (mapper.find() > 0) {
        for (TSqlORMapperIterator<TimetableObject> i(mapper); i.hasNext(); ) {
            array.append(QJsonValue(QJsonObject::fromVariantMap(Timetable(i.next()).toVariantMap())));
        }
    }
    return array;
}
