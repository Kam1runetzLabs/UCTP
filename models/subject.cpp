...
Subject::Subject() : TAbstractModel(), d(new SubjectObject()) {
  // set the initial parameters
}

Subject::Subject(const Subject &other) : TAbstractModel(), d(other.d) {}

Subject::Subject(const SubjectObject &object)
    : TAbstractModel(), d(new SubjectObject(object)) {}
...
int Subject::id() const { return d->id; }

QString Subject::subjectName() const { return d->subjectName; }

void Subject::setSubjectName(const QString &subjectName) {
  d->subjectName = subjectName;
}

int Subject::subjectType() const { return d->subjectType; }

void Subject::setSubjectType(int subjectType) { d->subjectType = subjectType; }
  ...
  if (mapper.find() > 0) {
    for (TSqlORMapperIterator<SubjectObject> i(mapper); i.hasNext();) {
      array.append(QJsonValue(
          QJsonObject::fromVariantMap(Subject(i.next()).toVariantMap())));
    }
  }
  return array;
}

TModelObject *Subject::modelData() { return d.data(); }

const TModelObject *Subject::modelData() const { return d.data(); }
