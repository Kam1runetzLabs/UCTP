...
Classroom::Classroom() : TAbstractModel(), d(new ClassroomObject()) {
  // set the initial parameters
}

Classroom::Classroom(const Classroom &other) : TAbstractModel(), d(other.d) {}

Classroom::Classroom(const ClassroomObject &object)
    : TAbstractModel(), d(new ClassroomObject(object)) {}
...
int Classroom::id() const { return d->id; }

QString Classroom::classroomName() const { return d->classroomName; }

void Classroom::setClassroomName(const QString &classroomName) {
  d->classroomName = classroomName;
}

int Classroom::classroomType() const { return d->classroomType; }
  ...
  if (mapper.find() > 0) {
    for (TSqlORMapperIterator<ClassroomObject> i(mapper); i.hasNext();) {
      array.append(QJsonValue(
          QJsonObject::fromVariantMap(Classroom(i.next()).toVariantMap())));
    }
  }
  return array;
}

TModelObject *Classroom::modelData() { return d.data(); }

const TModelObject *Classroom::modelData() const { return d.data(); }
