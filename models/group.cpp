...
Group::Group() : TAbstractModel(), d(new GroupObject()) {
  // set the initial parameters
}

Group::Group(const Group &other) : TAbstractModel(), d(other.d) {}

Group::Group(const GroupObject &object)
    : TAbstractModel(), d(new GroupObject(object)) {}
...
int Group::id() const { return d->id; }

QString Group::groupName() const { return d->groupName; }

void Group::setGroupName(const QString &groupName) { d->groupName = groupName; }
  ...
  if (mapper.find() > 0) {
    for (TSqlORMapperIterator<GroupObject> i(mapper); i.hasNext();) {
      array.append(QJsonValue(
          QJsonObject::fromVariantMap(Group(i.next()).toVariantMap())));
    }
  }
  return array;
}

TModelObject *Group::modelData() { return d.data(); }

const TModelObject *Group::modelData() const { return d.data(); }
