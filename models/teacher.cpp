...
QString Teacher::teacherName() const
{
...
QJsonArray Teacher::getAllJson()
{
    ...
    if (mapper.find() > 0) {
        for (TSqlORMapperIterator<TeacherObject> i(mapper); i.hasNext(); ) {
            array.append(QJsonValue(QJsonObject::fromVariantMap(Teacher(i.next()).toVariantMap())));
        }
    }
    return array;
}

TModelObject *Teacher::modelData()
{
    return d.data();
}

const TModelObject *Teacher::modelData() const
{
    return d.data();
}
